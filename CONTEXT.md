...
## Order
The billing and shipping envelope a customer submits. An Order is a **container**: it holds one or more Items and carries the customer's contact info, **Fulfillment Method** (Pickup or Mail-In), shipping address, and payment/deposit records. An Order has no status of its own — its displayed status is a rollup derived from its Items' statuses (e.g. "least-advanced item" or an explicit summary like "2 of 3 items in progress").
